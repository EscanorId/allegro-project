...
Bullet create_bullet(char* bullet_path, PointFloat coord, float angle, float speed, float damage) {
    ...
    if (!bullet.image) {
        printf("Failed to load bullet bitmap: %s\n", bullet_path);
    }
...
bool update_bullet(Bullet* bullet, enemyNode* enemyList, Map* map) {
    ...
    */
    ...
    int tile_y = (int)(bullet->coord.x / TILE_SIZE);
    int tile_x = (int)(bullet->coord.y / TILE_SIZE);
    if (tile_x < 0 || tile_y < 0 || tile_x >= map->col || tile_y >= map->row || !isWalkable(map->map[tile_y][tile_x])) {
        return true;
    }
    ...
    while (cur != NULL) {
        ...
        */
        ...
        //    bullet->coord.x <= enemyCoord.x + TILE_SIZE &&
        //    bullet->coord.y >= enemyCoord.y &&
        //    bullet->coord.y <= enemyCoord.y + TILE_SIZE) {
        //    hitEnemy(&cur->enemy, bullet->damage, bullet->angle); // Asumsi hitEnemy menerima pointer ke musuh dan damage peluru
        //    return true;
        //}
        int enemyTileX1 = enemyCoord.x;
        int enemyTileY1 = enemyCoord.y;
        int enemyTileX2 = enemyCoord.x + TILE_SIZE - 1;
        int enemyTileY2 = enemyCoord.y + TILE_SIZE - 1;

        if (bullet->coord.x >= enemyTileX1 && bullet->coord.x <= enemyTileX2 && //check if it is inside the enemy tile or not
            bullet->coord.y >= enemyTileY1 && bullet->coord.y <= enemyTileY2) {
            hitEnemy(cur, bullet->damage, bullet->angle); //knockback follows the angle of the bullet, damage comes from bullet's damage
            return true;
        }
...
void destroy_bullet(Bullet* bullet) {
    if (bullet->image) {
        printf("Destroying bullet bitmap...\n");
        al_destroy_bitmap(bullet->image);
        bullet->image = NULL;
    }
    else {
        printf("Warning: Attempted to destroy a NULL bullet bitmap!\n");
    }
}
...
void updateBulletList(BulletNode* dummyhead, enemyNode* enemyList, Map* map) {
    if (!enemyList || !enemyList->next) {
        // Jika musuh habis, tidak perlu update bullet
        return;
    }
    ...
    while (cur != NULL) {
        ...
        if (shouldDelete) {
            ...
            free(cur);
            cur = prev->next; // Tetap di posisi yang sama
        }
        else {
            prev = cur; // Hanya pindah ke node berikutnya jika tidak ada penghapusan
            cur = cur->next;
        }
    }
}
