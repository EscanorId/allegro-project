...
bool updateEnemy(Enemy* enemy, Map* map, Player* player) {
    ...
    if (enemy->status == DYING) {
        ...
        */
        if (enemy->death_animation_tick > 0) {
            enemy->animation_tick = (enemy->animation_tick + 2) % 72; //16px * 9 sprite
            enemy->death_animation_tick--; //subtract 
        }
        else return true; //return true when the enemy is dead (enemy disappears)
...
void drawEnemy(Enemy* enemy, Point cam) {
    ...
    if (enemy->status == ALIVE) {
        ...
        */
        ...
        int flag = enemy->dir == RIGHT ? 1 : 0; //facing right or left
        int tint_red = enemy->knockback_CD > 0 ? 255 : 0; //tint red if there is cooldown
...
void hitEnemy(Enemy* enemy, int damage, float angle) {
    ...
    */
    ...
    // if health is below zero, transition to dying status
    if (enemy->health <= 0) {
        enemy->status = DYING;
        enemy->death_animation_tick = 32; 
        enemy->animation_tick = 0;
    }
