...
additionalScene scene_create(int x, int y, int w, int h, const* default_image_path)
{
	additionalScene scene;
	memset(&scene, 0, sizeof(additionalScene));
	
	scene.default_img = al_load_bitmap(default_image_path);
	if (!scene.default_img) {
...
void draw_scene(additionalScene scene) {
	ALLEGRO_BITMAP* _img = scene.default_img;
...
void update_scene(additionalScene* scene) {
...
void destroy_scene(additionalScene* scene) {
	al_destroy_bitmap(scene->default_img);
}
